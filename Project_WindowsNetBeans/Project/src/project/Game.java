...
public class Game {
    ...
    public void writeLog(Board parent, Board curr, boolean team) {
        ...
        try {
            if (parent.checkEnPassant(team) != null && parent.checkBoardEquality(parent.checkEnPassant(team).getBoard(), curr.getBoard())) {//if an en passant move was made
                if (writer != null) {
                    writer.write(Integer.toString(turnNum));
                    if (team) {
                ...
                Coordinate[] move = getLastMove(parent, curr);
    ...
    public void closeLogCheckMate(String t) {
        try {
            writer.write("Checkmate, " + t + " wins!");
            writer.flush();
            writer.close();
        } catch (FileNotFoundException e) {
            System.out.println("File Not Found");
    ...
    public Coordinate[] getLastMove(Board parent, Board child) {
    ...
    public Board nextMove(Board b, boolean currTeam) {
        ...
        if (b.children.isEmpty()) {//if this team is unable to move
            ...
            Board aiMove = ai.alphaBeta(b, currTeam, 0, 2, Integer.MIN_VALUE, Integer.MAX_VALUE);
