...
public class Board {
    ...
    public Board() {//initial chess board
        ...
        for (int i = 0; i < 2; i++) {
            ...
            boardState[4][i * 7] = new Queen(team);
            boardState[3][i * 7] = new King(team);
            team = false;
        }
        fitness = calcFitness();
    }
    ...
    /**
     * Creates all boards that are possible as a result of a legal move on this
     * board
     *
    ...
    public void calculateChildren(boolean team) {
        ...
        for (int y = 0; y < 8; y++) {//for each spot on the board
            for (int x = 0; x < 8; x++) {
                if (y == yCoor && boardState[x][y] != null && boardState[x][y].getClass().getName().equals("project.Pawn") && boardState[x][y].team == team) {//Adds the possible promotions
                    Piece[][] childBoard = deepCopy(boardState);//duplicate the parent board to new array
                    Piece curr = new Queen(team);//get duplicate of piece that is currently moving
                    childBoard[x][y] = null;//remove the piece from its origional location
                    childBoard[x][y + deltaY] = curr;//put the duplicate in the new position
                    childBoard[x][y + deltaY].hasMoved = true;//this piece has now moved
                    Board child = new Board(childBoard);//create new board object with childBoard
                    child.parent = this;
                    tempChildren.add(child);
                    childBoard = deepCopy(boardState);//duplicate the parent board to new array
                    curr = new Bishop(team);//get duplicate of piece that is currently moving
                    childBoard[x][y] = null;//remove the piece from its origional location
                    childBoard[x][y + deltaY] = curr;//put the duplicate in the new position
                    childBoard[x][y + deltaY].hasMoved = true;//this piece has now moved
                    child = new Board(childBoard);//create new board object with childBoard
                    child.parent = this;
                    tempChildren.add(child);
                    childBoard = deepCopy(boardState);//duplicate the parent board to new array
                    curr = new Knight(team);//get duplicate of piece that is currently moving
                    childBoard[x][y] = null;//remove the piece from its origional location
                    childBoard[x][y + deltaY] = curr;//put the duplicate in the new position
                    childBoard[x][y + deltaY].hasMoved = true;//this piece has now moved
                    child = new Board(childBoard);//create new board object with childBoard
                    ...
                    tempChildren.add(child);
                    childBoard = deepCopy(boardState);//duplicate the parent board to new array
                    curr = new Rook(team);//get duplicate of piece that is currently moving
                    childBoard[x][y] = null;//remove the piece from its origional location
                    childBoard[x][y + deltaY] = curr;//put the duplicate in the new position
                    childBoard[x][y + deltaY].hasMoved = true;//this piece has now moved
                    child = new Board(childBoard);//create new board object with childBoard
                    child.parent = this;
                    tempChildren.add(child);
                } else if (boardState[x][y] != null && boardState[x][y].team == team) {//if there is a piece belonging to the specified team at this location
    ...
    public boolean checkCheckmate(boolean team) {
        ...
        if (checkCheck(team)) {//if the team is currently in check
            ...
            if (this.children.isEmpty()) {//if there are no legal moves that this team can make
                checkmate = true;
            }
        }
        return checkmate;
        ...
        for (int y = 0; y < 8; y++) {//for each spot on the board
            for (int x = 0; x < 8; x++) {
                if (boardState[x][y] != null && boardState[x][y].team == team) {//if there is a piece belonging to the specified team at this location
                    if (boardState[x][y].getClass().getName().equals("project.King")) {//if this piece is the king
    ...
    /**
     * Checks if there are any en passant moves possible and returns the board
     * on which the move has been made
     *
     * @param team the team currently moving
    ...
    public Board checkEnPassant(boolean team) {
        ...
        for (int y = 0; y < 8; y++) {//for each spot on the board
            for (int x = 0; x < 8; x++) {
                if (boardState[x][y] != null && boardState[x][y].team == team) {//if there is a piece belonging to the specified team at this location
                    if (boardState[x][y].getClass().getName().equals("project.Pawn")) {//if that piece is a pawn
                        if (team && y == 4) {//if its a white piece in position
                            ...
                            if (x + 1 <= 7) {
                                if ((boardState[x + 1][y] != null) && (boardState[x + 1][y].getClass().getName().equals("project.Pawn")) && (!boardState[x + 1][y].team)) {
    ...
    /**
     * Checks if there are any castling moves which can be made, returns a
     * linked list with the Boards which have those moves, if possible, empty
     * otherwise
     *
     * @param team the team currently moving
     * @return A linked list of boards of all possible castling moves, empty if
     * not possible
     */
    public LinkedList<Board> checkCastling(boolean team) {
        ...
        for (int y = 0; y < 8; y++) {//for each spot on the board
            for (int x = 0; x < 8; x++) {
                if (boardState[x][y] != null && boardState[x][y].team == team) {//if there is a piece belonging to the specified team at this location
                    if (boardState[x][y].getClass().getName().equals("project.Rook") && !boardState[x][y].hasMoved) {//if there is a rook that hasn't moved yet here
                        //left
                        int tempX = x;
    ...
    /**
     * this method gets all coordinates which the enemy can reach in their next
     * move. Useful for castling and check
     *
    ...
    public int calcFitness() {
        LinkedList<Coordinate> whiteThreatened = getThreats(true);
        LinkedList<Coordinate> blackThreatened = getThreats(false);
        int tempFitness = 0;
        tempFitness += whiteThreatened.size();
        tempFitness -= blackThreatened.size();
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (boardState[i][j] != null) {
                    tempFitness = boardState[i][j].team ? boardState[i][j].value : -boardState[i][j].value;
                    if (blackThreatened.contains(new Coordinate(i, j))) {
                        tempFitness += boardState[i][j].value;
                    }
                    if (whiteThreatened.contains(new Coordinate(i, j))) {
                        tempFitness -= boardState[i][j].value;
                    }
                }
